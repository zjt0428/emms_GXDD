...
/*		BusinessMessage bm = new BusinessMessage();
	...
	public void deleteChange(Dispatch dispatch) {
		...
		for (DispatchEquip ce : dispatch.getDispatchEquipSet()) {
			...
			filter.addConjunctFilter("Q_dispatchEquipId_L_EQ", ce.getDispatchEquipId()+"");
			List<EquipFlow> f = equipFlowDao.getAll(filter);
			if(f.size()>0) {
				QueryFilter filter1 = new QueryFilter();
				filter1.addConjunctFilter("Q_flowId_L_EQ", f.get(0).getFlowId()+"");
				List<EquipDiary> d = equipDiaryDao.getAll(filter1);
				if(d.size()>0) {
					equipDiaryDao.remove(d.get(0));
				}
				filter1 = null;
				equipFlowDao.remove(f.get(0));
			}
			filter = null;
			e = null;
		}
