...
//	});
	...
	this.safetyMonitorSettleListGrid = new SafetyMonitorSettleListGrid({
		contractEquipBriefGrid : this.contractEquipBriefGrid,
	...
	saveFormData : function() {
		...
		for (var i = 0; i < this.contractEquipBriefGrid.getStore().getCount(); i++) {
			...
			if (Ext.isEmpty(r.quantity)) {
				Ext.Msg.alert("【合同签订设备型号】页签信息不完整", "第<span style='color:red'>[" + (i + 1) + "]</span>条记录<span style='color:red'>租用数量</span>未填写!");
				return;
			}
			if (Ext.isEmpty(r.rentStandard)) {
				Ext.Msg.alert("【合同签订设备型号】页签信息不完整", "第<span style='color:red'>[" + (i + 1) + "]</span>条记录<span style='color:red'>租金标准</span>未填写!");
				return;
			}
		}
		for(var i = 0; i < this.safetyMonitorSettleListGrid.getStore().getCount(); i++){
			var r = this.safetyMonitorSettleListGrid.getStore().getAt(i).data;
			if (Ext.isEmpty(r.equipSpecificName)) {
				Ext.Msg.alert("【安全监控系统费用】页签信息不完整", "第<span style='color:red'>[" + (i + 1) + "]</span>条记录<span style='color:red'>设备型号</span>未填写!");
				return;
			}
			if (Ext.isEmpty(r.rentStandard)) {
				Ext.Msg.alert("【安全监控系统费用】页签信息不完整", "第<span style='color:red'>[" + (i + 1) + "]</span>条记录<span style='color:red'>租金标准</span>未填写!");
				return;
			}
		}
		for(var i = 0; i < this.contractInoutFreeGrid.getStore().getCount(); i++){
			var r = this.contractInoutFreeGrid.getStore().getAt(i).data;
			if (Ext.isEmpty(r.equipSpecificName)) {
				Ext.Msg.alert("【进出场费用】页签信息不完整", "第<span style='color:red'>[" + (i + 1) + "]</span>条记录<span style='color:red'>设备型号</span>未填写!");
				return;
			}
			if (Ext.isEmpty(r.rent)) {
				Ext.Msg.alert("【进出场费用】页签信息不完整", "第<span style='color:red'>[" + (i + 1) + "]</span>条记录<span style='color:red'>租金标准</span>未填写!");
				return;
			}
		}
		for(var i = 0; i < this.contractOperatorFreeGrid.getStore().getCount(); i++){
			var r = this.contractOperatorFreeGrid.getStore().getAt(i).data;
			if (Ext.isEmpty(r.equipSpecificName)) {
				Ext.Msg.alert("[操作人员费用]页签信息不完整", "第<span style='color:red'>[" + (i + 1) + "]</span>条记录<span style='color:red'>设备型号</span>未填写!");
				return;
			}
			if (Ext.isEmpty(r.rent)) {
				Ext.Msg.alert("[操作人员费用]页签信息不完整", "第<span style='color:red'>[" + (i + 1) + "]</span>条记录<span style='color:red'>租金标准</span>未填写!");
				return;
			}
		}
		var fileAttaches = this.getForm().findField("file_attaches").getValue();
		if(!fileAttaches){
			Ext.Msg.alert("合同信息不完整", "合同附件不能为空");
			return;
		}
